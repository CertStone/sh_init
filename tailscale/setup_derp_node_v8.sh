...
# ==============================================================================
# Tailscale DERP 节点自动化部署脚本 (v8 - 2025-06-27)
#
# 功能:
# 1. 安装 Docker 和 Docker-Compose.
# 2. 可选地为 Docker 配置国内镜像源以加速.
# 3. 自动适配主流 Debian & Ubuntu 版本，安装 Tailscale.
# 4. 允许用户选择 Let's Encrypt 或本地自签名证书.
# 5. 生成符合现代安全标准 (SANs) 的自签名证书，解决 x509 报错.
# 6. 直接生成 derper 默认期望的 <域名>.crt 文件名.
# 7. 保持 docker-compose.yml 干净，移除多余环境变量.
# 8. (修正) 根据最佳实践，在生成的 derpMap 中使用域名作为 HostName.
# ==============================================================================
...
install_dependencies() {
    ...
    if command_exists tailscale; then
        ...
        info "正在安装 Tailscale..."
        apt-get update -y
        if ! command_exists lsb_release; then apt-get install -y lsb-release; fi
        OS_DISTRO=$(lsb_release -is | tr '[:upper:]' '[:lower:]')
        OS_CODENAME=$(lsb_release -cs)
        if [ "$OS_DISTRO" != "debian" ] && [ "$OS_DISTRO" != "ubuntu" ]; then error "不支持的操作系统: $OS_DISTRO."; fi
...
configure_docker_mirror() {
    ...
    case "$choice" in
        [nN][oO]|[nN]) info "跳过 Docker 镜像源配置。"; return ;;
        *) ;;
    esac
    ...
    if [ -f "$DAEMON_JSON_FILE" ] && grep -q "$MIRROR_URL" "$DAEMON_JSON_FILE"; then
        info "Docker 镜像源已经配置为 $MIRROR_URL，跳过。"; return
    fi
...
EOF
    ...
    if ! systemctl restart docker; then error "重启 Docker 服务失败。"; fi
    info "Docker 服务重启成功。"
}
...
authenticate_tailscale() {
    ...
    if tailscale status &> /dev/null | grep -q "Logged in"; then
        ...
        echo; tailscale up --accept-dns=false; echo
...
EOF
    ...
    info "正在通过 Docker Compose 启动 DERP 服务..."
...
final_instructions() {
    ...
    # 注意：这里不再需要自动检测 IP 了，因为 HostName 直接使用域名
